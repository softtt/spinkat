...
function updateCartSummary(json)
{
	...
	for (i in product_list)
	{
		...
		var current_price_class ='price product-price';
		var price_reduction = '';
		if (reduction && typeof(initial_price) !== 'undefined')
		{
			...
			{
				initial_price_text = '<li class="old-price">' + initial_price + '</li>';
	...
	$('.shipping_label').show();
	$('#total_shipping').removeClass('wrap-text');

	var cart_rule_free_shipping = false;

	if (json.discounts) {
		json.discounts.forEach(function (e) {
			if (e.free_shipping == '1') {
				cart_rule_free_shipping = true;
			}
		})
	}

	if ((json.total_shipping > 0 || json.carrier.is_free == "0") && !cart_rule_free_shipping)
	{
		$('#total_shipping').addClass('wrap-text');
		if (json.total_shipping > 0) {
			$('.shipping_label').show();
			if (priceDisplayMethod !== 0)
			...
			$('.shipping_label').hide();
			$('#total_shipping').html(shippingCouldBeCalculated);
		}
	}
	else
	{
