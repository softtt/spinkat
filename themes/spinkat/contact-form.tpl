...
{else}
	...
	<form action="{$request_uri}" method="post" id="contact-form-box" class="contact-form-box" enctype="multipart/form-data">
		<fieldset>
			<h3 class="page-subheading">{l s='send a message'}</h3>
			<div class="clearfix">
				<div class="col-xs-12 col-md-3">
					<p class="form-group">
						<label for="name">{l s='Name'}</label><sup> *</sup>
						{if isset($customerThread.name)}
							<input class="form-control grey" type="text" id="name" name="name" value="{$customerThread.name|escape:'html':'UTF-8'}" readonly="readonly" />
						{else}
							<input class="form-control grey" type="text" id="name" name="name" value="{$name|escape:'html':'UTF-8'}" />
						{/if}
					</p>

					<p class="form-group">
						<label for="email">{l s='Email address'}</label><sup> *</sup>
					...
					<p class="form-group">
						<label for="subject">{l s='Subject'}</label>
						<input class="form-control grey" type="text" id="subject" name="subject" value="{$subject|escape:'html':'UTF-8'}" />
					</p>
					{*
					{if !$PS_CATALOG_MODE}
			...
			<div class="submit">
				<button type="submit" name="submitMessage" id="submitMessage" class="button btn btn-default button-medium"><span>{l s='Send'}</span></button>
			</div>
		</fieldset>
	</form>
{/if}
