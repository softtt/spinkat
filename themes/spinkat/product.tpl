...
<div itemscope itemtype="https://schema.org/Product">
    <meta itemprop="url" content="{$link->getProductPageLink($product)}">
    <div class="primary_block product_image_price_block row">
        {*
        {if !$content_only}
            <div class="container">
        ...
        *}
        ...
        <div class="pb-left-column {if $content_only}col-md-12{else}col-md-9{/if}">

            <div class="block_shadow">
                <div class="display-on-mobile block_shadow">
                    {if isset($product_manufacturer) && $manufacturer_image != 'ru-default'}
                        <div class="manufacturer">
                            {if isset($manufacturer_country_iso_code) && $manufacturer_country_iso_code}
                                <span class="flag">
                                    <span class="flag-icon {$manufacturer_country_iso_code}"></span>
                                </span>
                            {/if}
                            <span class="title">
                                <img src="{$img_manu_dir}{$manufacturer_image|escape:'html':'UTF-8'}.jpg" alt="{$product_manufacturer->name|escape:'html':'UTF-8'}" />
                            </span>
                        </div>
                    {/if}

                    <h2 itemprop="name" class="product_model_title">{$product->name|escape:'html':'UTF-8'}</h2>
                </div>

                {if ($product->is_new || $product->is_series)}
                    <span class="new-box" {if $product->is_series}style="display: none;"{/if}>
                    ...
                    <div id="image-block" class="clearfix">
                        {if $have_image}
                            <span id="view_full_size">
                                ...
                                {else}
                                    <img id="bigpic" {if $content_only}class="disable_clicks"{/if} itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html':'UTF-8'}" title="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}" width="{$largeSize.width}" height="{$largeSize.height}"/>
                                    {if $content_only}
                                        <script>
                                            $(function() {
                                                $('.disable_clicks').on('click', function(e) {
                                                    e.preventDefault();
                            ...
                            <div id="thumbs_list">
                                ...
                                {if isset($images)}
                                    {foreach from=$images item=image name=thumbnails}
                                        ...
                                        {/if}
                                        <li id="thumbnail_{$image.id_image}" {if $image.id_image == $cover.id_image}data-cover="1"{/if} {if $smarty.foreach.thumbnails.last} class="last"{/if}>
                                            <a
                                                {if $jqZoomEnabled && $have_image && !$content_only}
                                                ...
                                                {else}
                                                    href="{$link->getImageLink($product->link_rewrite, $imageIds)|escape:'html':'UTF-8'}" data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"
                                                {/if}
                                                data-bigpic-src="{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}"
                                                title="{$imageTitle}"
                                            >
                    ...
                    {if isset($images) && count($images) > 1}
                        <p class="resetimg clear no-print">
                            <span id="wrapResetImages" style="display: none;">
                                <a href="{$link->getProductPageLink($product)|escape:'html':'UTF-8'}" data-id="resetImages">
                ...
                <div class="center-column col-md-5">
                    <div class="display-on-desktop">
                        ...
                        <h1 itemprop="name" class="product_model_title">{$product->name|escape:'html':'UTF-8'}</h1>
                    </div>
        ...
        {if !$content_only}
            <!-- pb-right-column-->
            <div class="pb-right-column col-md-3">
                <div class="block_shadow">
                    ...
                    <form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
                        ...
                        <div class="box-info-product">
                            ...
                            <div class="product_attributes clearfix" style="display: none; visible: hidden;">
                                <!-- quantity wanted -->
                                {if !$PS_CATALOG_MODE}
                                                            ...
                                                            <ul id="color_to_pick_list" class="clearfix">
                                                                ...
                                                                {foreach from=$group.attributes key=id_attribute item=group_attribute}
                                                                    ...
                                                                    <li{if $group.default == $id_attribute} class="selected"{/if}>
                                                                        <a href="{$link->getProductPageLink($product)|escape:'html':'UTF-8'}" id="color_{$id_attribute|intval}" name="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}"{if !$img_color_exists && isset($colors.$id_attribute.value) && $colors.$id_attribute.value} style="background:{$colors.$id_attribute.value|escape:'html':'UTF-8'};"{/if} title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}">
                                                                            {if $img_color_exists}
                            ...
                            <div class="box-cart-bottom">
                                <div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
                                    <p id="add_to_cart" class="buttons_bottom_block no-print">
                                        <button type="submit" name="Submit" class="button product_page_add_to_cart_button">
                            ...
                            <div class="feedback-form-block">
                                <div>
                                    <a id="feedback-link" href="#product_feedback_form">{l s='Задать вопрос эксперту'}</a>
                                </div>
                            </div>
                            ...
                            {if ($product->is_series || isset($gift))}
                                <div class="product-gift" {if $product->is_series}style="display: none;"{/if}>
                                    <span>
                                        {if (!$product->is_series && isset($gift))}
                                            <a class="quick-view" href="{$gift->link|escape:'html':'UTF-8'}" rel="{$gift->link|escape:'html':'UTF-8'}">
                                                <img src="{$gift->banner|escape:'html':'UTF-8'}" alt="gift banner">
                                                <!-- <img src="{$img_dir}gift.png"> -->
                                            </a>
                                        {else}
                                            <a class="quick-view" href="" rel="">
                                                <img src="{$img_dir}gift.png" alt="gift banner">
                                                <!-- <img src=""> -->
                                            </a>
                                        {/if}
                                    </span>
                            ...
                            {if ($product->is_series || $free_delivery)}
                                <div class="product-free-delivery" {if $product->is_series}style="display: none;"{/if}>
                                    <span>
                                        <img src="/img/cr/7.jpg" alt="free delivery banner">
                                    </span>
                                </div>
                            {/if}
    ...
    {if $product->is_series && isset($models) && count($models) && isset($attr_groups)}
        {include file="$tpl_dir./models-table.tpl" models=$models attr_groups=$attr_groups}
    {/if}

    <div id="tab_information" class="block_shadow">

        <!-- Nav tabs -->
        <ul class="nav nav-tabs" role="tablist">

            {if $product->is_series || (!$product->is_series && $product->description)}
                <li role="presentation" class="description"><a href="#description" aria-controls="description" role="tab" data-toggle="tab">{l s='Description'}</a></li>
            {/if}

            {if !$content_only}
                {if isset($accessories) && $accessories}
                    <li role="presentation" class="accessories"><a href="#accessories" aria-controls="accessories" role="tab" data-toggle="tab">{l s='Accessories'}</a></li>
                {/if}

                {$HOOK_PRODUCT_TAB}

                {if $product->is_series || (!$product->is_series && $product->video)}
                    <li role="presentation" class="video"><a href="#video" aria-controls="video" role="tab" data-toggle="tab">{l s='Video'}</a></li>
                {/if}

                {if isset($attachments) && $attachments}
                    <li role="presentation" class="attachments"><a href="#attachments" aria-controls="attachments" role="tab" data-toggle="tab">{l s='Attachments'}</a></li>
                {/if}
            {/if}
        </ul>
        ...
        {if $product->is_series || (!$product->is_series && $product->description)}
            <div role="tabpanel" class="tab-pane" id="description">
                <div>
                    ...
                    <div class="model_description"></div>
                </div>
            </div>
        {/if}
                            ...
                            <ul id="bxslider" class="bxslider clearfix">
                                {foreach from=$accessories item=accessory name=accessories_list}
                                    {if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) && $accessory.available_for_order && !isset($restricted_country_mode)}
                                        {assign var='accessoryLink' value=$link->getProductPageLink($accessory.id_product, $accessory.link_rewrite, $accessory.category)}
    ...
    <script>
        $(document).ready(function() {
            $('#tab_information .nav-tabs li:visible:first a').tab('show')
        });
    </script>
...
<div style="display:none">
    <form id="product_feedback_form" action="/contacts" method="post" class="product-feedback-form-box" enctype="multipart/form-data">
        <fieldset>
            <h2 class="">{l s='Задать вопрос эксперту'}</h2>
            <div class="clearfix feedback_content">
                <label>{l s='Имя'}</label><sup> *</sup>
                <input class="form-control grey" type="text" name="name" />

                <label>{l s='Email'}</label><sup> *</sup>
                <input class="form-control grey" type="email" name="from" />

                <label>{l s='Тема'}</label>
                <input class="form-control grey" type="text" name="subject" />

                <input type="hidden" name="id_product" value="{$product->id|intval}" />
                <input type="hidden" name="id_product_attribute" />
                <input type="hidden" name="back" value="1">

                <label>{l s='Сообщение'}</label><sup> *</sup>
                <textarea class="form-control" id="message" name="message"></textarea>

                <p class="error validation" style="display: none;">Введены некорректные данные</p>

                <p class="submit">
                    <button type="submit" name="submitMessage" class="btn button button-medium"><span>{l s='Отправить'}</span></button>
                </p>
            </div>
        </fieldset>
    </form>
    ...
    <script type="text/javascript">
    $(function() {
        $('#product_feedback_form').on('submit', function(e) {
            if (!$(this).find('input[name=name]').val() ||
                !$(this).find('input[name=from]').val() ||
                !$(this).find('textarea[name=message]').val()
            ) {
                e.preventDefault();
                $(this).find('.error.validation').show();
            } else {
                $(this).find('.error.validation').hide();
            }
        });
    });
    </script>
...
{addJsDef product_name=$product->name|escape:'html':'UTF-8'}
