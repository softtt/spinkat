...
<div id="layered_block_left" class="layered_block_left">
	<div class="title_block block_shadow">Фильтр</div>
	<div class="block_content block_shadow">
				...
				*}
								...
								{if $filter.filter_type == 0}
									...
									<li>
										...
										<div class="layered_slider_container">
											<div class="layered_slider" id="layered_{$filter.type}_slider" data-type="{$filter.type}" data-format="{$filter.format}" data-unit="{$filter.unit}" data-link-rewrite="{if isset($filter.filter_rewrite_slug)}{$filter.filter_rewrite_slug}{/if}"></div>
										</div>
									</li>
								{else}
									{if $filter.filter_type == 1}
				...
				{if isset($selected_filters) && $n_filters > 0}
					<div class="reset_filters">
						<a id="reset_filters" class="light-button" href="#/show-all">{l s='Reset filters' mod='blocklayered'}</a>
					</div>
				{/if}
			</div>
			<input type="hidden" name="id_category_layered" value="{$id_category_layered}" />
			<input type="hidden" name="id_series_layered" value="{$id_series_layered}" />
			{foreach from=$filters item=filter}
