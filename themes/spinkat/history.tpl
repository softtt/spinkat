...
<div class="block_shadow box">
					...
					{foreach from=$orders item=order name=myLoop}
						<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
							...
							<td class="history_detail">
								<a class="light-button pull-left" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
									<span>
										{l s='Details'}
									</span>
								</a>
								{if isset($opc) && $opc}
									<a class="link-button pull-right p-a-5" href="{$link->getPageLink('order-opc', true, NULL, "submitReorder&id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
								{else}
									<a class="link-button pull-left" href="{$link->getPageLink('order', true, NULL, "submitReorder&id_order={$order.id_order|intval}")|escape:'html':'UTF-8'}" title="{l s='Reorder'}">
								{/if}
	...
	<ul class="footer_links clearfix">
		<li>
			<a class="light-button" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
					{l s='Back to Your Account'}
			</a>
		</li>
		<li>
			<a class="light-button" href="{$base_dir}">
				{l s='Home'}
			</a>
		</li>
	</ul>
