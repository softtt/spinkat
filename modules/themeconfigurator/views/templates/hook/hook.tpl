...
<div id="htmlcontent_{$hook|escape:'htmlall':'UTF-8'}"{if $hook == 'footer'} class="footer-block col-xs-12 col-sm-4"{/if}>
	<ul class="htmlcontent-home clearfix row">
			...
			{/if}
					...
					{/if}
						{if $hItem.image}
							...
							<img src="{$link->getMediaLink("`$module_dir`img/`$hItem.image`")}" class="item-img {if $hook == 'left' || $hook == 'right'}img-responsive{/if}" title="{$hItem.title|escape:'htmlall':'UTF-8'}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" {if $hItem.image_w}width="{$hItem.image_w|intval}"{/if} {if $hItem.image_h}height="{$hItem.image_h|intval}"{/if}/>
						{/if}
		...
		{/foreach}
		<li class="col-xs-12 mobile-banner-img"><img src="{$img_ps_dir}spinkat_specials_mobile_banner.jpg" alt="Акция: 10% скидка при покупке комплекта спиннинг + катушка"></li>
	</ul>
</div>
{/if}
