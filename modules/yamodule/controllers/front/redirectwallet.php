...
class YamoduleRedirectWalletModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (empty($code)) {
            ...
            $response = API::getAccessToken(
                ...
                $code,
                $this->context->link->getModuleLink('yamodule', 'redirect', array(), true),
                Configuration::get('YA_P2P_KEY')
            );
            ...
            if ($token == '') {
                ...
                if ($this->log_on) {
                    $this->module->logSave('wallet_redirect: '.$this->module->l('Type wallet'));
                }
                $auth_url = API::buildObtainTokenUrl(
                    Configuration::get('YA_P2P_IDENTIFICATOR'),
                    $this->context->link->getModuleLink('yamodule', 'redirect_wallet', array(), true),
                    $scope
                );

                if ($this->log_on) {
                    $this->module->logSave('wallet_redirect: url = '.$auth_url);
                }
            ...
            switch ($request_payment->status) {
                case 'success':
                    if ($this->log_on) {
                        $this->module->logSave('wallet_redirect: '.$this->module->l('Request success'));
                    }
                    $this->context->cookie->ya_encrypt_token
                        = urlencode($this->module->cryptor->encrypt($token));
                    $this->context->cookie->ya_encrypt_RequestId
                        = urlencode($this->module->cryptor->encrypt($request_payment->request_id));
                ...
                case 'refused':
                    if ($this->log_on) {
                        $this->module->logSave('wallet_redirect: '.$this->module->l('Request refused'));
                    }
    ...
    public function updateStatus(&$resp)
    {
        if ($resp->status == 'success') {
            ...
            if ($this->log_on) {
                $this->module->logSave('wallet_redirect: Error '.$this->module->descriptionError($resp->error));
            }
