...
require_once _PS_MODULE_DIR_ . 'ph_simpleblog/models/SimpleBlogAuthor.php';
...
class ph_simpleblog extends Module
{
    ...
    public function prepareModuleSettings()
    {
        ...
        **/
        ...
        foreach (Language::getLanguages(true) as $lang)
            $parent_tab->name[$lang['id_lang']] = $this->l('Materials');
        ...
        $tab->name = array();
        foreach (Language::getLanguages(true) as $lang)
            $tab->name[$lang['id_lang']] = $this->l('Sections');
    ...
    public function hookDisplayHeader($params)
    {
        ...
        $this->context->controller->addCSS($this->_path.'css/simpleblog_home_blocks.css', 'all');
    ...
    public function hookModuleRoutes($params)
    {
        ...
        $authors_slug = Configuration::get('PH_BLOG_AUTHORS_SLUG');
        ...
        $my_routes = array(
            ...
            **/
            ...
            'module-ph_simpleblog-authorsingle' => array(
                'controller' => 'authorsingle',
                'rule' => $authors_slug.'/{rewrite}',
                'keywords' => array(
        ...
        // Add top level categories to routing, and remove blog_slug from links.
        $top_categories = SimpleBlogCategory::getSimpleFirstLevelCategories($context->language->id);
        $category_routes = array();

        foreach ($top_categories as $category)
        {
            $link = $category['link_rewrite'];

            // Category list
            $category_routes['module-ph_simpleblog-'.$link] = array(
                'controller' => 'category',
                'rule' =>       $link,
                'keywords' => array(
                ),
            ...
            // Category pagination
            $category_routes['module-ph_simpleblog-'.$link.'page'] = array(
                'controller' => 'categorypage',
                'rule' => $link.'/page/{p}',
                'keywords' => array(
                    'p' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*', 'param' => 'p'),
            ...
            // Category blog post
            $category_routes['module-ph_simpleblog-'.$link.'single'] = array(
                'controller' => 'single',
                'rule' =>       $link.'/{rewrite}',
                'keywords' => array(
                    'rewrite' =>        array('regexp' => '[_a-zA-Z0-9-\pL]*', 'param' => 'rewrite'),
        ...
        $result = array_merge($my_routes, $category_routes);
    ...
    {
        $categories = SimpleBlogCategory::getCategories($this->context->language->id, true, true, true);

        foreach($categories as $key => $category) {
            $categories[$key]['posts'] = SimpleBlogPost::getPosts($this->context->language->id, 3, $category['id']);
        }

        $shop_name = Configuration::get('PS_SHOP_NAME');

        $this->context->smarty->assign(compact('categories', 'shop_name'));
    ...
    public function hookDisplayTopFooter1($params)
    {
        ...
        return $this->display(__FILE__, 'top-footer.tpl');
    }

    public function hookDisplayTopFooter2($params)
    {
        return $this->hookDisplayTopFooter1($params);
    }
    ...
    {
        $this->prepareProductTabsInformation();
    ...
    public function prepareProductTabsInformation()
    {
        ...
        $articles_category = SimpleBlogCategory::getByRewrite('articles');
        if ($articles_category)
            $articles = SimpleBlogPost::getProductPosts($this->context->language->id, $product->id, $articles_category->id);

        $surveys_category = SimpleBlogCategory::getByRewrite('surveys');
        if ($surveys_category)
            $surveys = SimpleBlogPost::getProductPosts($this->context->language->id, $product->id, $surveys_category->id);

        $blog_category = SimpleBlogCategory::getByRewrite('blog');
        if ($blog_category)
            $blog = SimpleBlogPost::getProductPosts($this->context->language->id, $product->id, $blog_category->id);

        $this->context->smarty->assign(compact('articles', 'surveys', 'blog'));
