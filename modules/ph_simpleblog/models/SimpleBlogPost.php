...
class SimpleBlogPost extends ObjectModel
{
    ...
    **/
    ...
    public $simpleblog_author = null;
    public $simpleblog_author_url;

    public $display_author = true;
    public $display_tags = true;
    ...
    public static $definition = array(
        ...
        'fields'                        => array(
            ...
            'id_simpleblog_author'      => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
            ...
            'link_rewrite'              => array('type' => self::TYPE_STRING, 'lang' => true, 'required' => true, 'size' => 255),
    ...
    public function __construct($id_simpleblog_post = null, $id_lang = null, $id_shop = null)
    {
        ...
        $this->display_author = SimpleBlogCategory::displayAuthor($this->id_simpleblog_category);
        $this->display_tags = SimpleBlogCategory::displayTags($this->id_simpleblog_category);
        ...
        if ($this->id_simpleblog_author > 0) {
            $this->simpleblog_author = new SimpleBlogAuthor($this->id_simpleblog_author, $id_lang);
            $this->simpleblog_author_url = SimpleBlogAuthor::getLink($this->simpleblog_author->link_rewrite);
            $this->author_thumb = $this->simpleblog_author->image_thumb;
        }
    }
    ...
    public static function getPosts($id_lang, $limit = 10, $id_simpleblog_category = null, $page = null, $active = true, $orderby = false, $orderway = false, $exclude = null, $featured = false, $author = false, $id_shop = null, $filter = false, $selected = array(), $check_access = true, $id_simpleblog_author = 0, $id_simpleblog_tag = 0)
    {
        $context = Context::getContext();
        ...
        if ($id_simpleblog_tag)
            $sql->innerJoin('simpleblog_post_tag', 'sbpt', 'sbpt.id_simpleblog_post = sbp.id_simpleblog_post
                             AND sbpt.id_simpleblog_tag = '.(int)$id_simpleblog_tag);
        ...
        if (isset($id_simpleblog_author) && (int)$id_simpleblog_author > 0)
        {
            $sql->where('sbp.id_simpleblog_author = '.(int)$id_simpleblog_author);
        }
        ...
        if (sizeof($result))
        {
            foreach ($result as &$row)
            {
                ...
                $row['display_author'] = SimpleBlogCategory::displayAuthor($row['id_simpleblog_category']);
                $row['display_tags'] = SimpleBlogCategory::displayTags($row['id_simpleblog_category']);
                ...
                if ($row['id_simpleblog_author'])
                {
                    $author = new SimpleBlogAuthor($row['id_simpleblog_author'], $id_lang);

                    $row['author_name'] = $author->name;
                    $row['author_url'] = SimpleBlogAuthor::getLink($author->link_rewrite);
                    $row['author_thumb'] = $author->image_thumb;
                }
            }
        }
        else
    ...
    {
        if ($category)
            return Context::getContext()->link->getModuleLink('ph_simpleblog', $category.'single', array('rewrite' => $rewrite));
        else
    ...
    public static function getPageLink($page_nb, $type = false, $rewrite = false, $tag = false)
    {
        ...
        if ($type == 'category')
        {
            ...
            {
                if ($tag)
                    return SimpleBlogCategory::getLink($rewrite, $id_lang, null, array('tag' => $tag));
                else
            ...
            if (isset($rewrite))
            {
                ...
                return $url.$dispatcher->createUrl('module-ph_simpleblog-'.$rewrite.'page', $id_lang, $params);
            }
        }
    ...
    public static function filter($id_simpleblog_category, $id_simpleblog_tag, $id_lang = null, $limit = 10, $page = null)
    {
        return self::getPosts(
            ...
            array(), #selected
            true, #check_access
            0, #id_simpleblog_author
            $id_simpleblog_tag
        );
    }

    public static function getPostsNb($id_simpleblog_category = null, $id_simpleblog_tag = null)
    {
        $sql = 'SELECT COUNT(*) FROM `'._DB_PREFIX_.'simpleblog_post` p';

        if ($id_simpleblog_tag)
        {
            $sql .= ' INNER JOIN `'._DB_PREFIX_.'simpleblog_post_tag` t
                      ON t.id_simpleblog_post = p.id_simpleblog_post
                      AND t.id_simpleblog_tag = ' . $id_simpleblog_tag;
        }

        if ($id_simpleblog_category)
            $sql .= ' WHERE `id_simpleblog_category` = ' . $id_simpleblog_category;

        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue($sql);
    }

    public static function getProductPosts($id_lang, $id_product, $id_category = null)
    {
        $sql = "SELECT id_simpleblog_post FROM `ps_simpleblog_post` WHERE `id_product` LIKE '%" . $id_product . "%'";

        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $post_ids = array();

        foreach ($result as $key => $post_id)
            $post_ids[] = $post_id['id_simpleblog_post'];

        $posts = array();

        if ($post_ids) {
            $posts = SimpleBlogPost::getPosts($id_lang, 10, $id_category, null, true, false, false, null, false, false, null, 'IN', $post_ids, true);
        }
