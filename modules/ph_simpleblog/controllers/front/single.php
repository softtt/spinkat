...
class PH_SimpleBlogSingleModuleFrontController extends ModuleFrontController
{
	...
	public function initContent()
	{
		...
		**/
		...
		if (!$this->SimpleBlogPost->meta_description) {
			$without_html_tags = strip_tags($this->SimpleBlogPost->content);
			$substring = mb_substr($without_html_tags , 0 ,160);
			$this->context->smarty->assign('meta_description', $substring);
		} else {
		...
		elseif ($this->SimpleBlogPost->allow_comments == 3)
			$allow_comments = SimpleBlogCategory::areCommentsAllowed($this->SimpleBlogPost->id_simpleblog_category);
		else
			$allow_comments = false;
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('submitNewComment') && Tools::getValue('id_simpleblog_post'))
		{
			...
			if (Tools::getValue('comment_content') && Validate::isGenericName(Tools::getValue('comment_content')))
                ...
                $errors[] = $this->module->l('Не указан текст комментария');
            ...
            else
            {
			    ...
			    if ($comment->add())
			    {
			    	...
			    	$link = $this->context->link->getModuleLink('ph_simpleblog', $SimpleBlogPost->category_rewrite.'single', array('confirmation' => $confirmation, 'rewrite' => $SimpleBlogPost->link_rewrite));
