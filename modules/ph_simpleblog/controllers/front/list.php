	...
	public function initContent()
  	{
	    ...
	    if ($this->sb_category != '')
	    {
	    	...
	    	$blogLayout = SimpleBlogCategory::getPostsListLayout($SimpleBlogCategory->id);
	    	...
	    	{
	    		$meta_description = $SimpleBlogCategory->meta_description;
				if ($page > 1) {
                    $meta_description .= ' (страница - '.$page.')';
				}
			...
			if ($tag_id = Tools::getValue('tag'))
		    {
		    	$posts = SimpleBlogPost::filter($SimpleBlogCategory->id, $tag_id, $id_lang, $this->posts_per_page, $page);
		    	$posts_count = SimpleBlogPost::getPostsNb($SimpleBlogCategory->id, $tag_id);
			    $this->context->smarty->assign('tag_id', $tag_id);
		    }
		    else
		    {
		    	$posts = SimpleBlogPost::getPosts($id_lang, $this->posts_per_page, $SimpleBlogCategory->id, $page);
				$posts_count = SimpleBlogPost::getPostsNb($SimpleBlogCategory->id);
		    }

	    	$this->assignPagination($this->posts_per_page, $posts_count);
	    ...
	    else
	    {
	    	...
	    	// @todo: More flexible
	    	$meta_title = Configuration::get('PH_BLOG_MAIN_TITLE', Context::getContext()->language->id) ? Configuration::get('PH_BLOG_MAIN_TITLE', Context::getContext()->language->id) : '';
	    ...
	    // Set grid columns based on category or global grid display.
	    $gridHtmlCols = '';
