...
class AdminSimpleBlogPostsController extends ModuleAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->_select = 'sbcl.name AS `category`, sbpt.name AS `post_type`, COUNT(sbpc.id_simpleblog_comment) AS `comments`';
        ...
        $this->_join .= 'LEFT JOIN `'._DB_PREFIX_.'simpleblog_comment` sbpc ON (sbpc.`id_simpleblog_post` = a.`id_simpleblog_post`)';

        $this->_group = 'GROUP BY a.id_simpleblog_post';
            ...
            if (Tools::getValue('id_simpleblog_post', 0))
            {
                ...
                if ($this->display == 'edit')
                {
                    $this->page_header_toolbar_btn['view_details'] = array(
                        'href' => self::$currentIndex.'&detailssimpleblog_post&id_simpleblog_post='.$SimpleBlogPost->id.'&token='.$this->token,
                        'desc' => $this->l('Post comments list', null, null, false),
    ...
    {
        if (($id = Tools::getValue('id_simpleblog_post')))
        {
            $post = new SimpleBlogPost($id, $this->context->employee->id_lang);

            $comments = SimpleBlogComment::getComments($id, true, false);

            foreach ($comments as &$comment)
            {
                $comment['id_simpleblog_post'] = $post->id;
                $comment['id_simpleblog_comment'] = $comment['id'];
            ...
            $fields_list = array(
                ...
                'id_simpleblog_post' => array(
                    ...
                    'align' => 'center',
                    'width' => 25,
                    // 'filter_key' => 'sbpl!id_simpleblog_post',
                ),
                'post_title' => array(
            ...
            $helper->module = $this;
            $helper->table = 'simpleblog_comment';
            $helper->identifier = 'id_simpleblog_comment';
            $helper->title = $this->l('Comments of post: ').' '.$post->title;
            $helper->token = Tools::getAdminTokenLite('AdminSimpleBlogComments');
            $helper->currentIndex = $this->context->link->getAdminLink('AdminSimpleBlogComments', false);
    ...
    public function renderForm()
    {
        ...
        **/
        ...
        $authors = SimpleBlogAuthor::getAuthors($this->context->language->id, true);

        $available_authors = array(
            0 => array(
                'id' => 0,
                'name' => $this->l('-- Choose an author --')
            )
        );
    ...
    */
    ...
    public function ajaxProcessSearchProducts()
    {
        ...
        $sql = 'SELECT p.`id_product`, pl.`link_rewrite`, pl.`name`, p.`reference`, pl.`name` as product_name
                FROM `'._DB_PREFIX_.'product` p
                '.Shop::addSqlAssociation('product', 'p').'
                LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (p.`id_product` = pl.`id_product` '.Shop::addSqlRestrictionOnLang('pl').')
