...
{
    const ATTRIBUTE_TYPE_SLIDER = 3;

    private $products;
    private $nbr_products;
    private $page = 1;

    private $is_series_page = false;
    ...
    public function __construct()
    {
        ...
        if (isset($this->context->controller) && $this->context->controller->php_self == 'series')
            $this->is_series_page = true;

        if (Tools::getIsset('id_series'))
            $this->id_series = Tools::getValue('id_series');
    }
...
//            $smarty->assign('meta_keywords', rtrim($category_title.', '.$keywords.', '.$category_metas['meta_keywords'], ', '));
        ...
        $this->context->controller->addJQueryUI('ui.touch-punch');
    ...
    public static function indexProductPrices($id_product, $smart = true)
    {
        ...
        foreach ($shop_list as $id_shop)
        {
            ...
            foreach ($currency_list as $currency) {
                ...
                $prices = $product->getCombinationsPrices($id_shop, (int)$cookie->id_lang, $currency['id_currency']);

                if ($prices['min'] < $min_price[$currency['id_currency']]) {
                    $min_price[$currency['id_currency']] = $prices['min'];
                }

                if ($prices['max'] > $max_price[$currency['id_currency']]) {
                    $max_price[$currency['id_currency']] = $prices['max'];
                }
            }
            ...
            self::indexProductCombinationsPrices($id_product, $id_shop, $smart, $currency_list, $max_tax_rate);
        }
    }
        ...
        foreach ($_GET as $key => $value) {
            if (substr($key, 0, 8) == 'layered_') {
                ...
                if (isset($res[1])) {
                    ...
                    } else if (substr($res[1], 0, 13) == 'id_attribute_') {
                        $selected_filters[$res[0]] = $tmp_tab;
    ...
    public function getProductByFilters($selected_filters = array())
    {
        ...
        foreach ($selected_filters as $key => $filter_values)
        {
            ...
            if ($this->isFilterKeyAttributeSlider($key)) {
                $id = str_replace('id_attribute_', '', $key);
                $id = str_replace('_slider', '', $id);

                if (isset($filter_values[0]) && isset($filter_values[1])) {
                    $query_filters_where .= ' AND p.id_product IN (
                        ...
                        WHERE a.id_attribute_group = '.$id.'
                        AND al.name >= '.$filter_values[0].' AND al.name <= '.$filter_values[1].')';
                }
            }
        }
        ...
        else
        {
            ...
            if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true)
            {
                ...
                $ordering = Tools::getProductsOrder('by', Tools::getValue('orderby'), true).' '.Tools::getProductsOrder('way', Tools::getValue('orderway'));

                if (Tools::getValue('orderby') && Tools::getValue('orderby') == 'price') {
                    $ordering = 'lpi.price_min ' . Tools::getProductsOrder('way', Tools::getValue('orderway'));
                }
                ...
                ORDER BY '.$ordering.' , cp.id_product'.
    ...
    public function getFilterBlock($selected_filters = array())
    {
        ...
        Db::getInstance()->execute('CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_restriction ENGINE=MEMORY
                                                    SELECT DISTINCT cp.id_product, p.id_manufacturer, pa.id_product_attribute, pa.price, product_shop.condition, p.weight FROM '._DB_PREFIX_.'category_product cp
                                                    INNER JOIN '._DB_PREFIX_.'category c ON (c.id_category = cp.id_category AND
                                                    '.(Configuration::get('PS_LAYERED_FULL_TREE') ? 'c.nleft >= '.(int)$parent->nleft.'
                                                    AND c.nright <= '.(int)$parent->nright : 'c.id_category = '.(int)$id_parent).'
                                                    AND c.active = 1)
                                                    ...
                                                    '.(($id_tag && $tag_where) ? ('AND pa.id_product_attribute IN ('.$tag_where.')') : '').'
                                                    AND product_shop.`visibility` IN ("both", "catalog")', false);

        // d('CREATE TEMPORARY TABLE '._DB_PREFIX_.'cat_restriction ENGINE=MEMORY
        //                                             SELECT DISTINCT cp.id_product, p.id_manufacturer, pa.id_product_attribute, pa.price, product_shop.condition, p.weight FROM '._DB_PREFIX_.'category_product cp
        //                                             INNER JOIN '._DB_PREFIX_.'category c ON (c.id_category = cp.id_category AND
        //                                             '.(Configuration::get('PS_LAYERED_FULL_TREE') ? 'c.nleft >= '.(int)$parent->nleft.'
        //                                             AND c.nright <= '.(int)$parent->nright : 'c.id_category = '.(int)$id_parent).'
        ...
        Db::getInstance()->execute('ALTER TABLE '._DB_PREFIX_.'cat_restriction ADD PRIMARY KEY (id_product, id_product_attribute),
        ...
        if ($id_tag) {
            $filtered_product_attribute_ids = '';
            $product_attributes_array = Db::getInstance()->executeS('SELECT id_product_attribute FROM '._DB_PREFIX_.'cat_restriction  WHERE id_product = ' . $this->id_series);

            foreach ($product_attributes_array as $product_attribute) {
                $filtered_product_attribute_ids .= $product_attribute['id_product_attribute'].',';
            }
            $filtered_product_attribute_ids = substr($filtered_product_attribute_ids, 0, -1);
        }
        // d($filtered_product_attribute_ids);
        ...
        foreach ($filters as $filter)
        {
            ...
            switch ($filter['type'])
            {
                case 'price':
                    if ($id_tag) {
                        $sql_query['select'] = 'SELECT p.`id_product`, MIN(p.`price`) as price_min, MAX(p.`price`) as price_max';
                ...
                case 'id_attribute_group':// attribute group
                    if ($filter['filter_type'] == self::ATTRIBUTE_TYPE_SLIDER) {
                        $sql_query['select'] = 'SELECT pa.id_attribute, pa.id_attribute_group, agl.public_name, MIN(al.name) as min, MAX(al.name) as max';
                        $sql_query['from'] = '
                        FROM '._DB_PREFIX_.'cat_restriction p';
                        $sql_query['join'] = 'LEFT JOIN `'._DB_PREFIX_.'layered_product_attribute` pa
                            ON (pa.id_product = p.id_product
                                AND pa.id_shop='.(int)$context->shop->id.'
                                AND pa.id_attribute_group = '.(int)$filter['id_value'].')';
                        $sql_query['join'] .= ' LEFT JOIN `'._DB_PREFIX_.'attribute_lang` al
                            ON (al.id_attribute = pa.id_attribute AND al.id_lang = '.(int)$id_lang.') ';
                        $sql_query['join'] .= ' LEFT JOIN `'._DB_PREFIX_.'attribute_group_lang` agl
                            ON (agl.id_attribute_group = pa.id_attribute_group AND agl.id_lang = '.(int)$id_lang.') ';
                        $sql_query['where'] = (($id_tag && $filtered_product_attribute_ids) ? (' WHERE pa.id_attribute IN (SELECT DISTINCT id_attribute FROM '._DB_PREFIX_.'product_attribute_combination WHERE id_product_attribute IN ('.$filtered_product_attribute_ids.'))') : '');
                        ...
                        $sql_query['where'] .=  (($id_tag && $filtered_product_attribute_ids) ? (' AND lpa.id_attribute IN (SELECT DISTINCT id_attribute FROM '._DB_PREFIX_.'product_attribute_combination WHERE id_product_attribute IN ('.$filtered_product_attribute_ids.'))') : '');
            ...
            foreach ($filters as $filter_tmp)
            {
                ...
                if (method_exists('BlockLayered', $method_name) &&
                    ($filter['type'] != 'price' &&
                     ...
                     $filter['type'] == $filter_tmp['type']) &&
                    !($filter['type'] == 'id_attribute_group' && $filter['filter_type'] == 3))
                {
            ...
            if ($this->is_series_page && $this->id_series) {
                $sql_query['where'] .= ' AND p.id_product = ' . $this->id_series;
            }
            ...
            switch ($filter['type'])
            {
                ...
                case 'id_attribute_group':
                    // Slider
                    if ($filter['filter_type'] == self::ATTRIBUTE_TYPE_SLIDER) {
                        $slider_attribute_array = array(
                            'type_lite' => 'id_attribute_' . $filter['id_value'],
                            'type' => 'id_attribute_' . $filter['id_value'],
                            'id_key' => $filter['id_value'],
                            'name' => '',
                            'slider' => true,
                            'max' => '0',
                            'min' => null,
                        ...
                        if (isset($products) && $products) {
                            foreach($products as $product) {
                                if (isset($product['min']) && is_null($slider_attribute_array['min'])) {
                                    $slider_attribute_array['min'] = $product['min'];
                                   $slider_attribute_array['values'][0] = $product['min'];
                                }

                                if (isset($product['min']) && $product['min'] > 0
                                    && $product['min'] < $slider_attribute_array['min']
                                ) {
                                   $slider_attribute_array['min'] = $product['min'];
                                   $slider_attribute_array['values'][0] = $product['min'];
                                }
                                ...
                                if (isset($product['max'])
                                    && $product['max'] > 0
                                    && $product['max'] > $slider_attribute_array['max']
                                ) {
                                   $slider_attribute_array['max'] = $product['max'];
                                   $slider_attribute_array['values'][1] = $product['max'];
                                }

                                if (isset($product['public_name'])) {
                                    $slider_attribute_array['name'] = $product['public_name'];
                                    $slider_attribute_array['filter_rewrite_slug'] = str_replace($this->getAnchor(), '_', Tools::link_rewrite($product['public_name']));
                                }
                            }
                        }
                        $key = 'id_attribute_' . $filter['id_value'] . '_slider';
                        if (isset($selected_filters[$key]) &&
                            isset($selected_filters[$key][0]) &&
                            isset($selected_filters[$key][1])
                        ) {
                            $selected_vars = $selected_filters[$key];
                            if ($selected_vars && $selected_vars[0] && $selected_vars[1]) {
                                $slider_attribute_array['values'][0] = $selected_vars[0];
                                $slider_attribute_array['values'][1] = $selected_vars[1];
                            }
                        }

                        if ($slider_attribute_array['min'] && $slider_attribute_array['max']) {
                            $filter_blocks[] = $slider_attribute_array;
                        }
        ...
        foreach ($filter_blocks as &$type_filter)
        {
            ...
            if (($type_filter['slider']) && ((int)$type_filter['max'] == (int)$type_filter['min']))  {
                $type_filter = false;
            }
        ...
        $cache = array(
            ...
            'id_series_layered' => (int)$this->id_series,
        );
    ...
    public function ajaxCall()
    {
        ...
        if (Tools::getIsset('id_series_layered') && Tools::getValue('id_series_layered')) {
            $this->is_series_page = true;
            $this->id_series = Tools::getValue('id_series_layered');
        }
        ...
        $selected_filters = $this->getSelectedFilters();
        $filter_block = $this->getFilterBlock($selected_filters);
        $this->getProducts($selected_filters, $products, $nb_products, $p, $n, $pages_nb, $start, $stop, $range, $id_tag);
        ...
        $id_category = Tools::getValue('id_category_layered', Configuration::get('PS_HOME_CATEGORY'));
        ...
        $page_metas = Meta::getCategoryMetas($id_category, (int)$cookie->id_lang, '');

        if ($this->is_series_page && $this->id_series) {
            $page_metas = Meta::getSeriesMetas($this->id_series, (int)$cookie->id_lang);
        }
...
//            $meta_title = $category_metas['meta_title'];
        ...
        $page_number_text = ((! Tools::getIsset('p')) ? ' (страница - ' . $p . ')' : "");
        $meta_title = $page_metas['meta_title'] . $page_number_text;
        $meta_description = $page_metas['meta_description'] . $page_number_text;

        $keywords = Tools::substr(Tools::strtolower($keywords), 0, 1000);
        if (!empty($keywords))
            $meta_keywords = rtrim($category_title.', '.$keywords.', '.$page_metas['meta_keywords'], ', ');
    ...
    public function getProducts($selected_filters, &$products, &$nb_products, &$p, &$n, &$pages_nb, &$start, &$stop, &$range, $id_tag = null)
    {
        global $cookie;

        if ($this->is_series_page && $this->id_series) {
            $models_filters = $this->getModelsFilters($selected_filters);
            $products = $this->getModelsByFilters($this->id_series, $models_filters, false, $id_tag);
            $nb_products = $this->getModelsByFilters($this->id_series, $models_filters, true);
        } else {
        ...
        foreach ($products as &$product)
        {
            if (($id_tag) && ($id_tag != 'undefined')) {
                $link_array = explode('#',$product['link']);
    ...
    /**
     * Get filtered models on Series page.
     *
     * @param  int $id_series
     * @param  array  $selected_filters
     * @return array $models Filtered models
     */
    public function getModelsFilters($selected_filters = array()) {
        $models_filters = array(
            'id_attributes' => array(),
            'quantity' => array(),
            'price' => array(),
        );
        ...
        foreach ($selected_filters as $key => $filter_values) {
            ...
            switch ($key)
            {
                case 'id_attribute_group':
                    ...
                    foreach ($filter_values as $filter_value) {
                        ...
                        $models_filters['id_attributes'][$filter_value_array[0]][] = (int)$filter_value_array[1];
                    }
                break;
                ...
                case 'price':
                    if (isset($selected_filters['price'])) {
                        if ($selected_filters['price'][0] !== '' || $selected_filters['price'][1] !== '') {
                            $models_filters['price']['min'] = (float)($selected_filters['price'][0]);
                            $models_filters['price']['max'] = (float)($selected_filters['price'][1]);
                        }
                    }
                break;
            }
            ...
            if ($this->isFilterKeyAttributeSlider($key)) {
                ...
                if ($id) {
                    $models_filters['id_attribute_sliders'] = array(
                        $id => $filter_values,
                    );
                }
            }
        ...
        return $models_filters;
    }

    public function getModelsByFilters($id_series, $models_filters = array(), $count = false, $id_tag = null)
    {
        $series = new Series($id_series, true, $this->context->language->id, $this->context->shop->id);

        $sql_join = '';
        $sql_where = '';

        if ($models_filters) {
            foreach ($models_filters as $key => $filter) {
                if ($key == 'id_attributes') {
                    if ($filter) {
                        $sql_join .= ' LEFT JOIN '._DB_PREFIX_.'product_attribute_combination pac
                            ON (pac.id_product_attribute = pa.id_product_attribute)';

                        foreach ($filter as $attr_group_id => $attr_ids) {

                            $sql_where .= ' AND pa.id_product_attribute IN (SELECT DISTINCT pa.id_product_attribute FROM `ps_product_attribute` pa
                                LEFT JOIN ps_product_attribute_combination pac ON(pac.id_product_attribute = pa.id_product_attribute)
                                LEFT JOIN ps_attribute a ON (a.id_attribute = pac.id_attribute)
                                WHERE pa.`id_product` = '.(int)$id_series.'
                                AND a.id_attribute_group = '.$attr_group_id.'
                                AND pac.id_attribute IN ('.implode(', ', $attr_ids).'))';
                        }
                    }
                } elseif ($key == 'price') {
                    $sql_join .= ' LEFT JOIN '._DB_PREFIX_.'layered_attribute_price_index lapi
                        ON (lapi.id_product_attribute = pa.id_product_attribute)';
                    if (isset($filter['min']) && $filter['min'] > 0) {
                        $sql_where .= ' AND lapi.price >= ' . $filter['min'];
                    }
                    if (isset($filter['max']) && $filter['max'] > 0) {
                        $sql_where .= ' AND lapi.price <= ' . $filter['max'];
                    }
                } elseif ($key == 'id_attribute_sliders') {
                    foreach ($filter as $attr_group_id => $values) {

                        $sql_where .= ' AND pa.id_product_attribute IN (
                            SELECT DISTINCT pa.id_product_attribute FROM `ps_product_attribute` pa
                            LEFT JOIN ps_product_attribute_combination pac ON(pac.id_product_attribute = pa.id_product_attribute)
                            LEFT JOIN ps_attribute a ON (a.id_attribute = pac.id_attribute)
                            LEFT JOIN ps_attribute_lang al ON (al.id_attribute = a.id_attribute)
                            WHERE pa.`id_product` = '.(int)$id_series.'
                            AND a.id_attribute_group = '.$attr_group_id.'
                            AND al.name >= '.$values[0].'
                            AND al.name <= '.$values[1].')';
        ...
        if ($count) {
            if ($sql_join || $sql_where) {
                $sql = 'SELECT COUNT(DISTINCT pa.id_product_attribute) FROM `'._DB_PREFIX_.'product_attribute` pa';
                $sql .= $sql_join;
                $sql .= ' WHERE pa.`id_product` = '.(int)$id_series;
                $sql .= $sql_where;
            } else {
                $sql = 'SELECT COUNT(DISTINCT pa.id_product_attribute) FROM `'._DB_PREFIX_.'product_attribute` pa WHERE pa.`id_product` = '.(int)$id_series;
            }
        ...
        if ($sql_join || $sql_where) {
            ...
            $sql = 'SELECT DISTINCT pa.id_product_attribute FROM `'._DB_PREFIX_.'product_attribute` pa WHERE pa.`id_product` = '.(int)$id_series;
        }

        $filter_attributes_ids = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);

        $filter_ids = array_map(
            function ($v) {return $v['id_product_attribute'];},
            $filter_attributes_ids
        );
        ...
        $models = $series->getModels($this->context->language->id, false, $this->page, $n, $filter_ids, $id_tag);

        return $models;
    }

    public static function indexProductCombinationsPrices($id_product, $id_shop, $smart = true, $currency_list = null)
    {
        if ($smart) {
            Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'layered_attribute_price_index` WHERE `id_product` = '.(int)$id_product.' AND `id_shop` = '.(int)$id_shop);
        }
        ...
        $combinations_ids = $product->getWsCombinations();

        if ($combinations_ids) {
            foreach ($combinations_ids as $key => $id_combination) {

                $id_product_attribute = $id_combination['id'];

                $price = 0;
                $values = array();

                foreach ($currency_list as $currency) {
                    $price = Product::priceCalculation($id_shop, (int)$id_product, $id_product_attribute, null, null, null,
                        $currency['id_currency'], null, null, false, 6, false, true, true,
                        $specific_price_output, true);
                ...
                Db::getInstance()->execute('
                    INSERT INTO `'._DB_PREFIX_.'layered_attribute_price_index` (id_product, id_product_attribute, id_currency, id_shop, price)
                    VALUES '.implode(',', $values).'
                    ON DUPLICATE KEY UPDATE id_product = id_product # avoid duplicate keys');
            }
        }
    ...
    private function isFilterKeyAttributeSlider($key = '')
    {
        return (substr($key, 0, strlen('id_attribute_')) == 'id_attribute_' &&
                substr($key, - strlen('_slider')) === '_slider');
    }
}
