...
<div class="form-group">
	...
	<label class="control-label col-lg-3">
		<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="													Загрузите изображение логотипа производителя с Вашего компьютера
		">
		Логотип
	</span>
</label>
...
<div class="col-lg-6">
	...
	<div class="form-group">
		<div class="col-sm-6">
			<input id="logo" type="file" name="logo" class="hide">
			<div class="dummyfile input-group">
				<span class="input-group-addon"><i class="icon-file"></i></span>
				<input id="logo-name" type="text" name="filename" readonly="">
				<span class="input-group-btn">
					<button id="logo-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
						<i class="icon-folder-open"></i> Добавить файл
					</button>
				</span>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$('#logo-selectbutton').click(function(e) {
				$('#logo').trigger('click');
			});
			...
			$('#logo-name').on('dragenter', function(e) {
				e.stopPropagation();
				e.preventDefault();
			});

			$('#logo-name').on('dragover', function(e) {
				e.stopPropagation();
				e.preventDefault();
			});

			$('#logo-name').on('drop', function(e) {
				e.preventDefault();
			...
			$('#logo').change(function(e) {
				if ($(this)[0].files !== undefined)
				{
					...
					$('#logo-name').val(name.slice(0, -2));
				}
else // Internet Explorer 9 Compatibility
{
	var name = $(this).val().split(/[\\/]/);
	$('#logo-name').val(name[name.length-1]);
}
});

			if (typeof logo_max_files !== 'undefined')
			{
				$('#logo').closest('form').on('submit', function(e) {
					if ($('#logo')[0].files.length > logo_max_files) {
						e.preventDefault();
