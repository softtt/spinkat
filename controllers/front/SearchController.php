...
class SearchControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if ($this->instant_search && !is_array($query)) {
            ...
            // Stop searching products if query is less than 3 symbols
            if (strlen($query) < 4) {
                $this->context->smarty->assign(array(
                    'products' => array(),
            ...
            $blogPostSearcher = new BlogSearch();
            if ($blogPostSearcher) {
                $blogPosts = $blogPostSearcher->initialize($query);
                $this->context->smarty->assign([
                    'blogPosts' => $blogPosts,
                ]);
            }

            $search = Search::exactSearch($this->context->language->id, $query, $this->p, $this->n, $this->orderBy, $this->orderWay, 'desc', false);

            if (is_array($search['result']) && count($search['result'])) {
