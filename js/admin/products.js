...
product_tabs['Combinations'] = new function(){
	...
	this.bindEdit = function(){
		$('#models_list').delegate('a.edit', 'click', function(e){
			e.preventDefault();
			e.stopPropagation();
		...
		function editProductAttribute (url, parent){
			$.ajax({
				...
				success: function(data) {
					...
					var short_description = data[0]['short_description'];
					var long_description = data[0]['long_description'];
					var top_description = data[0]['top_description'];
					var attribute_video = data[0]['attribute_video'];
					var is_new = data[0]['is_new'];
					var hide = data[0]['hide'];
	...
	this.bindDefault = function(){
		$('#models_list').delegate('a.default', 'click', function(e){
			e.preventDefault();
			self.defaultProductAttribute(this.href, this);
		});
	};

	this.cloneProductAttribute = function(url, item){
		$.ajax({
			url: url,
			data: {
				id_product: id_product,
				action: 'cloneProductAttribute',
				ajax: true
			},
			context: document.body,
			dataType: 'json',
			...
			success: function(data) {
				if (data.status == 'ok') {
					...
					$('#models_list').html(data.models_table);
				} else {
					showErrorMessage(data.message);
				}
			}
	...
	this.bindClone = function(){
		$('#models_list').delegate('a.clone', 'click', function(e){
			e.preventDefault();
			self.cloneProductAttribute(this.href, this);
		});
	};
	...
	this.bindDelete = function() {
		$('#models_list').delegate('a.delete', 'click', function(e){
			e.preventDefault();
	...
	ean, quantity, image, old_attr, id_product_attribute, default_attribute, eco_tax, upc, minimal_quantity, available_date, title, short_description,
									long_description, attribute_video, top_description, is_new, hide,tags)
	{
		var link = '';
		self.init_elems();
		...
		$('#combination_tags').tagify();
		$('#combination_tags').tagify('destroy').val(tags);

		$('#combination_tags').val(tags);
		var tagifyVar = $('#combination_tags').tagify();
		tagifyVar.tagify('inputField')
			.autocomplete(availableTags);
		/*{
			source:availableTags,
				position:{ of:
			$('#combination_tags').tagify('containerDiv') },
			close:function(event, ui) {
				$('#combination_tags').tagify('add'); }
		}*/
		...
		if (typeof(combination_images) != 'undefined' && typeof(combination_images[id_product_attribute]) != 'undefined')
		...
		getE('attribute_title').value = title;
		getE('attribute_short_description').value = short_description;
		getE('attribute_long_description').value = long_description;
		getE('attribute_video').value = attribute_video;
		getE('attribute_top_description').value = top_description;

		if (is_new == 1)
			getE('attribute_is_new').checked = true;
		else
			getE('attribute_is_new').checked = false;

		if (hide == 1)
			getE('attribute_hide').checked = true;
		else
			getE('attribute_hide').checked = false;

		tinySetup({
			editor_selector: "combination_rte"
		});
	...
	this.init_elems = function()
	{
		...
		$('#attribute_is_new').attr('checked', false);
		$('#attribute_hide').attr('checked', false);
